# control.pipelineContext - Controller for comodit pipeline contexts entities.
# coding: utf-8
#
...
class PipelineContextController(AbstractController):

    def __init__(self):
        super(PipelineContextController, self).__init__()
        self._doc = "History of pipeline execution"
    ...
    def _list_doc(self):
        return ActionDoc("list", "<org_name> <pipeline_name> ", """
        list all executions of pipeline.""")

    def _show_doc(self):
        return ActionDoc("show", "<org_name> <pipeline_name> <context_id> ", """
        show detail of execution of pipeline.""")
    ...
    def _print_show_completions(self, param_num, argv):
        if param_num < 2:
            ...
            contexts = self._client.pipelineContexts(argv[0], argv[1])
            for res in contexts:
    ...
    def _show(self, argv):
        context = self._client.pipelineContext(argv[0], argv[1], argv[2])
